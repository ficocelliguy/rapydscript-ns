version 0.9.6
=======================
* Added support for `tuple` as a distinct runtime type. Parenthesised tuple literals `(a, b, ...)` (including empty `()` and single-element `(x,)`) and the `tuple()` constructor now produce decorated arrays carrying the Python `.count()` and `.index()` methods, paren-style `str()`/`repr()`, and an `__eq__` that compares element-wise. `isinstance(x, tuple)` and `isinstance(x, list)` now discriminate between the two; tuple `+` tuple returns a tuple (via `ρσ_list_add` / `ρσ_op_add`), and tuple `*` int returns a tuple under `overload_operators`.
* Added support for Python printf-style `%` string formatting when the left operand is a string (e.g. `'%s=%d' % ('x', 1)`, `'%(name)s' % {'name': 'Bob'}`, `'%05d' % 42`). Full set of conversion types (`s`, `r`, `a`, `c`, `d`, `i`, `u`, `o`, `x`, `X`, `e`, `E`, `f`, `F`, `g`, `G`, `%`), flags (`#`, `0`, `-`, ` `, `+`), width / precision (including `*` to consume positional args), and mapping form are supported; augmented `s %= ...` works as well.
* Added support for Python `bytes` and `bytearray` builtins backed by plain JS arrays of integers in 0–255, plus the `b'...'` / `B'...'` / `rb'...'` bytes literal syntax. Includes construction from strings (`utf-8`, `latin-1`, `ascii`), lists, `Uint8Array`, or another bytes-like object; indexing, slicing, iteration, concatenation, repetition, containment; `.hex()` / `bytes.fromhex()`, `.decode()`, `.find()` / `.index()` / `.rfind()` / `.count()`, `.startswith()` / `.endswith()`, `.join()` / `.split()` / `.replace()`, `.strip()` / `.lstrip()` / `.rstrip()`, `.upper()` / `.lower()`, `.copy()`; and `bytearray`-only mutation methods (item/slice assignment, `.append()`, `.extend()`, `.insert()`, `.pop()`, `.remove()`, `.reverse()`, `.clear()`, `+=`).
